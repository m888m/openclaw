...
describe("scheduleRestartSentinelWake", () => {
  ...
  it("replays generated-media provenance through the owning session agent", async () => {
    await deliverQueuedSessionDelivery({
      ...
      entry: {
        ...
        sourceSessionId: "image-generate-task-1-session",
    ...
    expect(mocks.dispatchGatewayMethodInProcess).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        idempotencyKey: "image:task-1:agent-loop",
        expectedExistingSessionId: "agent:main:main",
      }),
      expect.objectContaining({
        expectFinal: true,
        forceSyntheticClient: true,
        agentHandoffCapability: expect.any(Object),
        agentHandoffSource: {
          sourceSessionKey: "image_generate:task-1",
          sourceSessionId: "image-generate-task-1-session",
        },
  ...
  it("dead-letters legacy inter-session replay without an exact source incarnation", async () => {
    await expect(
      deliverQueuedSessionDelivery({
        deps: {} as never,
        entry: {
          id: "session-delivery-legacy-media",
          kind: "agentTurn",
          sessionKey: "agent:main:main",
          message: "generated image ready",
          messageId: "image:legacy-task:agent-loop",
          enqueuedAt: 1,
          retryCount: 0,
          route: { channel: "discord", to: "channel:123", chatType: "channel" },
          inputProvenance: {
            kind: "inter_session",
            sourceSessionKey: "image_generate:legacy-task",
            sourceChannel: "webchat",
            sourceTool: "image_generate",
          },
          sourceReplyDeliveryMode: "automatic",
    ...
    ).rejects.toThrow("lacks an exact purpose-bound source incarnation");
    ...
    expect(mocks.markSessionDeliverySettlement).toHaveBeenCalledWith(
      expect.objectContaining({ id: "session-delivery-legacy-media" }),
      "moved-to-failed",
    );
  });
    ...
    await expect(
      deliverQueuedSessionDelivery({
        ...
        entry: {
          ...
          retryCount: 0,
          route: { channel: "discord", to: "channel:123", chatType: "channel" },
          sourceSessionKey: "image_generate:test-task",
          sourceSessionId: "image-generate-test-session",
          sourceChannel: "webchat",
          sourceTool: "image_generate",
          sourceReplyDeliveryMode: "automatic",
          expectedMediaUrls: ["/tmp/proof.png"],
